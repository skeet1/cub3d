...
/*   Updated: 2022/07/26 16:28:47 by mkarim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_map_assist(t_map *map, char *line, int *height, bool prev_isempty)
{
	if (!is_map(line))
		return (1);
	exit_free_if(is_map(ft_substr(line, 0,
				ft_strlen(line) - 1)) && prev_isempty,
	...
	if (!prev_isempty)
	{
		...
		map->map = ft_2darr_add(map->map, ft_substr(line, 0,
					ft_strlen(line) - 1));
		(*height)++;
	}
	return (0);
