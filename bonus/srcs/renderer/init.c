...
/*   Updated: 2022/07/24 15:28:45 by mkarim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_map   *init_rndr(t_map *map)
{
	...
	exit_free_if(!(map->txtr->north_vec = malloc(sizeof(*map->txtr->north_vec))), "Error:\n\tmalloc failed", map);
...
t_map	*new_img(t_map *map)
{
	...
	map->mlx->endian = 0;
	map->mlx->img_data = (int *)mlx_get_data_addr(map->mlx->img, &map->mlx->bpp, &map->mlx->size_line, &map->mlx->endian);
...
t_map	*init_mlx(t_map	*map)
{
	...
	exit_free_if(!(map->txtr->img_n = mlx_xpm_file_to_image(map->mlx->mlx, map->txtr->north, &map->txtr->w, &map->txtr->h)), "Error:\n\tmlx image creation failed", map);
	exit_free_if(!(map->txtr->img_s = mlx_xpm_file_to_image(map->mlx->mlx, map->txtr->south, &map->txtr->w, &map->txtr->h)), "Error:\n\tmlx image creation failed", map);
	exit_free_if(!(map->txtr->img_e = mlx_xpm_file_to_image(map->mlx->mlx, map->txtr->east, &map->txtr->w, &map->txtr->h)), "Error:\n\tmlx image creation failed", map);
	exit_free_if(!(map->txtr->img_w = mlx_xpm_file_to_image(map->mlx->mlx, map->txtr->west, &map->txtr->w, &map->txtr->h)), "Error:\n\tmlx image creation failed", map);
	map->txtr->north_data = (int *)mlx_get_data_addr(map->txtr->img_n, &map->txtr->bpp, &map->txtr->size_line, &map->txtr->endian);
	map->txtr->south_data = (int *)mlx_get_data_addr(map->txtr->img_s, &map->txtr->bpp, &map->txtr->size_line, &map->txtr->endian);
	map->txtr->west_data = (int *)mlx_get_data_addr(map->txtr->img_w, &map->txtr->bpp, &map->txtr->size_line, &map->txtr->endian);
	map->txtr->east_data = (int *)mlx_get_data_addr(map->txtr->img_e, &map->txtr->bpp, &map->txtr->size_line, &map->txtr->endian);
	map = new_img(map);
	return (map);
}
